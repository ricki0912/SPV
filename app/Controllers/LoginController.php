...
class LoginController
{
    ...
    public function validateUser()
    {
        ...
        if (!empty($_POST["user"]) and !empty($_POST["password"]) and !empty($_POST["role"])) {
            ...
            $user = User::where('dni', $_POST["user"])
                    ->where('rol', $_POST["role"])
            ...
            if (count($user)) {
                ...
                $_SESSION["role"]           = $userT[0]->rol;

                $_SESSION["nomapell"]=$userT[0]->nomapell;
                $_SESSION["email"]=$userT[0]->email;
                $_SESSION["telefono"]=$userT[0]->telefono;
                $_SESSION["cargo"]=$userT[0]->cargo;
                $_SESSION["rol"]           = $userT[0]->rol;
